...
public class AddCreditCardForm extends FWForm {
	// AddAccountController controller;
	FWTableModel model;

	public AddCreditCardForm(String title, IForm form, FWTableModel model) {
		super(title, form);
		this.model = model;
	...
	void JButtonOK_actionPerformed(java.awt.event.ActionEvent event) {
		try {
			...
			if (JRadioButton_Bronze.isSelected())
				acc.setCCType(CCType.BRONZE);
			else if (JRadioButton_Gold.isSelected())
				acc.setCCType(CCType.GOLD);
			else if (JRadioButton_Silver.isSelected())
				acc.setCCType(CCType.SILVER);
			acc.setExpiryDate(JTextField_ExpDate.getText().trim());
			controller.addCCAccount(acc);
			model.refreshData();
